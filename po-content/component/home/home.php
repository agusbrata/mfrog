						...
						if (strlen($_POST['password']) >= 6) {
							if (md5($_POST['password']) != md5($_POST['repassword'])) {
								...
								if ($count_user_name > 0) {
									...
									$subject = "Email Account Activation For M-Frog Indonesia";
									...
									$message = "<html>
										<body>
											...
											-----------<br />
											Hi $username,<br />
											Jika anda tidak pernah mendaftarkan akun di M-Frog Indonesia, silahkan untuk menghiraukan email ini.<br />
											Tetapi jika benar Anda telah membuat akun di M-Frog Indonesia, maka silahkan untuk mengklik tautan (link) di bawah ini untuk mengaktifkan akun Anda :<br /><br />
											...
											-----------<br />
											Hi $username,<br />
											If you have never registered account in M-Frog Indonesia, please to ignore this email.<br />
											But if you really are registered account in M-Frog Indonesia, please to click on a link below to activated yout account :<br /><br />
											<a href=\"$website_url/member/activation/$username/$passmd5\" title=\"Account Activation\">$website_url/member/activation/$username/$passmd5</a><br /><br />
...
/**
 * Router untuk menampilkan request halaman aktifasi nomor hp member.
 *
 * Router for display request in activation handphone member page.
 *
*/
$router->match('GET|POST', '/member/aktivasi', function() use ($core, $templates) {
	if ($core->posetting[17]['value'] == 'Y') {
		if (empty($_SESSION['namauser_member']) AND empty($_SESSION['passuser_member']) AND empty($_SESSION['login_member'])) {
			$alertmsg = '';
			$lang = $core->setlang('home', WEB_LANG);
								...
								if ($count_user_name > 0) {
									...
									$message = "<html>
										<body>
											...
											Jika anda tidak pernah mendaftarkan akun di M-Frog Indonesia, silahkan untuk menghiraukan email ini.<br />
											Tetapi jika benar Anda telah membuat akun di M-Frog Indonesia, maka silahkan untuk mengklik tautan (link) di bawah ini untuk mengaktifkan akun Anda :<br /><br />
											<a href=\"$website_url/member/aktivasi/$username/$passmd5\" title=\"Account aktivasi\">$website_url/member/activation/$username/$passmd5</a><br /><br />
											Setelah link tersebut diklik maka akun Anda telah diaktifkan dan telah terverifikasi, silahkan login dengan data berikut :<br /><br />
											--------------------<br />
											Username : $username<br />
											Password : $pass<br />
			...
			echo $templates->render('aktivasi', compact('lang'));
		} else {
			header('location:'.BASE_URL.'/member');
		}
	} else {
